...
function go(){
	...
	function countTime(second){
		...
		startClock = setInterval(function(){	
			...
			if( (++sec) > second ){
				var score = calculateWPM(sec,totalChar);
				saveScore(second, score);	
				stopApp();
				return 0;
				}
	...
	function calculateWPM(time,charters){
		return parseInt((charters/6)/(time/60));
		}
	...
	function saveScore(second,score){
		var min = second/60;

		console.log(min+'  lol  '+score)
		...
		$.ajax({
			...
			url:'/home/save_score',
	 		success:function (result){
	 				console.log('Your score saved to server')
	 		 	}
		})
	}
		// ***************The Main********************//
